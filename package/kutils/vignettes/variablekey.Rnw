...
and ``wide''. The \code{keyTemplate} function's argument \code{long}
can be set as \code{TRUE} or \code{FALSE}. If \code{FALSE}, the
key template will be in the wide. The essence of the difference is
that the wide format puts all of the required information into a one-row-per-variable
...
The changes to be made in the wide or long format keys are equivalent.
