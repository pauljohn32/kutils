...
\hline 
x6.c & integer & character & 1|2|3|4|5 & New York|Denver|Nashville|Los Angeles|Portland\tabularnewline
\hline 
...
\subsection{Step 3. Import the edited key}

keyImport() kutils

keyImport reads the key and creates a list with one item per new variable
to be created. 
...
be suitable to coerce. But if stingsAsFactors = FALSE, 
\end{enumerate}
