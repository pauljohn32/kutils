...
##'
##' Addresses the problem that R's update.packages does not
##' trigger the installation of packages that are added as new
##' requirements in existing packages. It checks for existence
##' of updates, ascertains whether those packages impose new
##' requirements, and installs any new requirements. Checks
##' installed packages for package dependencies, updates
...
##' packages may be updated, but if user is not root/administrator, there
##' will be a warning that packages were not updates because permissions
##' were lacking. This warning does not interfere with rest of purpose
##' of this function, since the new dependencies can be installed in a
##' place where the user has privileges, either by specifying libnew
##' as a full directory name or by setting it to NULL, which puts new
...
updatePackages <- function(ask = FALSE, checkBuilt = TRUE,
                           ...
                           repos = c("http://rweb.crmda.ku.edu/cran", "http://rweb.crmda.ku.edu/kran"),
                           ...)
{
